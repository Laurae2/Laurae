...
lightgbm.cv <- function(
  ...
  if ((!is.na(log_name)) & (verbose %in% c(-1, 2))) {
    sink(file = file.path(workingdir, log_name), append = log_append, split = as.logical(verbose + 1))
  }
  
  for (i in 1:length(folds_list)) {
    
    if (!((!is.na(log_name)) & (verbose %in% c(0, 1)))) {
      sink(file = stri_replace_last_fixed(file.path(workingdir, log_name), ".", paste0("_", i, ".")), append = log_append, split = as.logical(verbose))
    }
